...
module.exports = {
    ...
    async execute(interaction) {
		...
		console.log(searchStartOption)
		console.log(searchEndOption)

        // Define variables for today, next week, and result limit
        const today = new Date();
        const todayTimestamp = Math.floor(today.getTime() / 1000);
        const todayReadable = convertDateReadable(today);

        const nextWeek = new Date(today.getTime() + 7 * 24 * 60 * 60 * 1000);
        const nextWeekTimestamp = Math.floor(nextWeek.getTime() / 1000);
        const nextWeekReadable = convertDateReadable(nextWeek);
        ...
        try {
            ...
            responseBody.forEach(event => {
                ...
                description = description.length > descriptionMaxChars ? description.substring(0, descriptionMaxChars) + '...' : description;
